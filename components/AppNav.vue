...
<template>
  <header class="fixed inset-x-0 top-0 z-50">
    <div class="shell pt-3 md:pt-4">
      <div class="border border-ink/10 bg-[rgba(255,252,244,0.9)] px-4 py-3 shadow-soft backdrop-blur-xl md:rounded-full md:px-5">
        ...
        <div class="flex items-center gap-3">
          ...
          <button class="button-secondary px-4 lg:hidden" aria-label="打开导航" @click="open = !open">
            {{ open ? '关闭' : '菜单' }}
          </button>
        </div>
        </div>

        <div v-if="open" class="grid gap-3 border-t border-ink/10 pt-4 lg:hidden">
